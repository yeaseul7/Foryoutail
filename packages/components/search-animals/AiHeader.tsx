    ...
    <section className="mx-auto w-full max-w-5xl px-1 pb-8 pt-10 sm:px-4 sm:pb-12 sm:pt-14">
      <header className="text-center">
        <h1 className="text-3xl font-extrabold tracking-[-0.04em] text-[#4f8ed8] sm:text-4xl lg:text-[2.75rem]">
          {isEnglish ? 'Find a Shelter Pet That Looks Like Yours' : '사진으로 유기동물 검색'}
        </h1>
        <p className="mt-4 text-sm font-medium text-slate-700 sm:text-base">
          {isEnglish ? 'AI analyzes daily updated adoption listings to find the closest match.' : 'AI가 유기동물 공고를 기준으로 사진과 가장 비슷한 아이를 찾아줘요'}
        </p>
      </header>
      ...
      <div className="mx-auto mt-8 w-full max-w-[820px] p-0">
        ...
        <div className="mx-auto mt-4 grid w-full max-w-[410px] gap-4">
          <button
            type="button"
          ...
          >
            <input
              ...
              className="sr-only"
              aria-label={isEnglish ? 'Upload a dog or cat photo' : '강아지 또는 고양이 사진 업로드'}
            />
            {previewUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={previewUrl}
                alt={isEnglish ? 'Uploaded pet' : '업로드한 사진'}
      ...
      <div className="mt-6 flex flex-col items-center gap-3">
        ...
        >
          ...
          {searchLoading ? (isEnglish ? 'Searching...' : '검색 중...') : (isEnglish ? 'Find with AI' : 'AI로 찾기')}
        </button>
        {dailyAiUsed != null && (
          <p className="text-xs font-medium text-slate-500">
            {isEnglish ? `${Math.max(dailyLimit - dailyAiUsed, 0)} searches left today` : `오늘 ${Math.max(dailyLimit - dailyAiUsed, 0)}회 남았어요`}
          </p>
        )}
        {!modelReady && !searchLoading && onLoadModel && (
          ...
          >
            {isEnglish ? 'Load AI model' : 'AI 모델 로드하기'}
          </button>
        )}
      </div>
    </section>
